...
def parse_args():
    ...
    parser.add_argument("--wandb-project-name", type=str, default="ppo-maze-disc2cont", 
        help="WandB 项目名称")
...
class Agent(nn.Module):
    def __init__(self, envs): #envs is now MazeEnv
        ...
        self._directions = envs._directions
    ...
    def get_action_prob_and_value(self, x, action=None):
        ...
        discrete_dist = Categorical(logits=logits) # 使用 Categorical 分布
        probs = discrete_dist.probs  # 实际概率张量 [batch_size, 8]

        if action is None:
            action = discrete_dist.sample() # 从 Categorical 分布中采样动作
            
        # 构建连续动作高斯分布（批量计算）
        directions = self._directions.unsqueeze(0)  # [1,8,2]
        mu = torch.einsum('bi,bij->bj', probs, directions)  # 加权平均 [batch,2]
        
        # 计算协方差矩阵
        diffs = directions - mu.unsqueeze(1)  # [batch,8,2]
        cov = torch.einsum('bi,bik,bil->bkl', probs, diffs, diffs)  # [batch,2,2]
        cov += torch.eye(2, device=cov.device).unsqueeze(0) * 1e-6
        
        cont_dist =  MultivariateNormal(loc=mu, covariance_matrix=cov)
        # 计算离散动作action对应连续动作的概率
        cont_log_prob = cont_dist.log_prob(self._directions[action])
        discrete_logprob = discrete_dist.log_prob(action) 
        log_prob = cont_log_prob + discrete_logprob

        return action, log_prob, discrete_dist.entropy(), self.critic(x) # 返回动作，log 概率，熵和价值
...
if __name__ == "__main__":
    ...
    dirs = { # 定义实验相关的目录
        'base': f"runs/MazeDisc2Cont/{run_name}", # 实验 runs 根目录
        'checkpoints': f"runs/MazeDisc2Cont/{run_name}/checkpoints", # 检查点保存目录
        'best': f"runs/MazeDisc2Cont/{run_name}/best_models", # 最佳模型保存目录
        'config': f"runs/MazeDisc2Cont/{run_name}/config", # 配置文件保存目录
        'videos': f"runs/MazeDisc2Cont/{run_name}/videos" # 视频录制保存目录
    }
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                for param_name, param in agent.named_parameters():
                    ...
                    if torch.isinf(param).any():
                        print(f"Inf detected in {param_name}!")
