...
if __name__ == "__main__":
    ...
    dirs = { # 定义实验相关的目录
        'base': f"runs/{run_name}", # 实验 runs 根目录
        'checkpoints': f"runs/{run_name}/checkpoints", # 检查点保存目录
        'best': f"runs/{run_name}/best_models", # 最佳模型保存目录
        'config': f"runs/{run_name}/config", # 配置文件保存目录
        'videos': f"runs/{run_name}/videos" # 视频录制保存目录
    }
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            next_obs, reward, terminated, truncated, infos = envs.step(action.cpu().numpy()) # MazeEnv step returns terminated and truncated
