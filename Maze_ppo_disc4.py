...
if __name__ == "__main__":
    ...
    dirs = { # 定义实验相关的目录
        'base': f"runs/MazeDisc4/{run_name}", # 实验 runs 根目录
        'checkpoints': f"runs/MazeDisc4/{run_name}/checkpoints", # 检查点保存目录
        'best': f"runs/MazeDisc4/{run_name}/best_models", # 最佳模型保存目录
        'config': f"runs/MazeDisc4/{run_name}/config", # 配置文件保存目录
        'videos': f"runs/MazeDisc4/{run_name}/videos" # 视频录制保存目录
    }
    ...
    from env.Maze import Discrete4Maze
    step_length = 0.1
    envs = Discrete4Maze(num_envs=args.num_envs,
